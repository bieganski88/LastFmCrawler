...
class Geocoder(object):
    ...
    def __str__(self):
        ...
        '''
        print 'Ilosc zaladowanych danych wejsciowych:\
            {} koncertow'.format(len(self.data))
        print 'Ilosc przetworzonych danych gotowych do exportu:\
            {} koncertow'.format(len(self.results))
    ...
    def process(self):
        ...
        Wejscie dane zrodlowe z JSON, wyjscie te same dane wzbogacone
        o wspolrzedne. Pomocniczo korzysta z LookupTable.db
        '''
        # ile razy zostalo uzyte google maps api - za darmo tylko 2500 dziennie
        gm_api = 0
            ...
            for event in self.data:
                ...
                if len(rows) > 0:  # jest w bazie
                    ...
                    try:
                        ...
                        query = u'INSERT INTO GEOLOKALIZACJE VALUES\
                            (NULL, "{}", "{}", {}, {})'.format(country, city, lat, lng)
                ...
                geo_event = event[:]  # kopia listy
                geo_event.append(lat)
                geo_event.append(lng)

                self.results.append(geo_event)  # dodaje do wynikow
        ...
        print "Google Maps API zostalo uzyte: {} raz(y).".format(gm_api)
    ...
    def export_to_json(self):
        ...
        feature_collection = {'type': "FeatureCollection"} # element glowny pliku json
    ...
    def export_to_db(self, django=False):
        '''
        ...
        if django:
            ...
            conn = sqlite3.connect('{}/eventsGeom.db'.format(self._destpath))
        cur = conn.cursor()
        # czyszcze tabele
        ...
        for record in self.results:
            ...
            # insert do bazy danych
            try:
                query_part1 = u'INSERT INTO events_musicevents(id, artist, title,\
                    event_date, lineup, place, city, country, latitude, longitude) VALUES'
                query_part2 = u'(NULL, "{}", "{}", "{}", "{}", "{}", "{}", "{}",\
                    {}, {})'.format(artist, title, date, lineup, place, city, country, lat, lng)
    ...
    def export_to_django(self):
        ...
        '''
        self.export_to_db(True) # true - oznacza import do bazy django
